...
Apply all `must_haves` from read patterns and applicable global rules. Treat `golden_pattern` code as the implementation reference — match its structure and its `Modifier.semantics` usage, adapted to the project's existing component and theming conventions.
